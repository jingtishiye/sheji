...
<div class="userCenter_wrapper">
  <div class="userCenter_location"> <a href="index_yz.php">业主个人中心</a>&nbsp;&nbsp;>&nbsp;&nbsp;<a href="index_yz.php">业主信息</a>&nbsp;&nbsp;>&nbsp;&nbsp;<span>我的评价</span> </div>
<?php
include("yz_left.php");
?>
...
<div class="clear"></div>
   <div class="pageRightWrap">
            <div class="pR_tit_w">
                <div class="pR_tit">我的评价</div>
               
            </div>
           
<?php
$cnum_zb = mysql_num_rows(mysql_query("select id from tb_upl where  uid=$arrayuv[id]",$conn));
?>		
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
				...
				<?php
				$xxrs=mysql_query("select * from tb_upl where uid=$arrayuv[id] order by  id desc ",$conn);
				$J=1;
 ...
 <?php
       ...
       <a href="yz_pl_xx.php?id=<?php echo $array["id"]?>"  style="color:#ff0000;"><?php echo $bname?></a> 
                    </td>
				    <td class="td">
       <?php
...
<img src="<?php echo $array["uimg"]?>" style="border:1px solid #cccccc;"  width="50" />
				    ...
				    <td class="td"><input type="button" name="Submit3" value="查看" onclick="window.location.href='yz_pl_xx.php?id=<?php echo $array["id"]?>' "  class="btn"/>&nbsp;&nbsp;&nbsp;&nbsp;
				    <input type="button" name="Submit" value="删除" onclick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='yz_pl.php?delid=<?php echo $array["id"]?>&del=ok';}else{history.go(0);}"  class="btn"/></td>
				</tr>
				<?php
				$J++;
				 endwhile;
...
$rs=mysql_query("delete from tb_upl where uid='$arrayuv[id]' and id='$delid'",$conn);
echo "<script>window.location.href='yz_pl.php';</script>" ; 
