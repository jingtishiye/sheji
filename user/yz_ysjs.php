...
<?php
$cnum_zb = mysql_num_rows(mysql_query("select id from tb_uysj where  uid=$arrayuv[id]",$conn));
?>		
      <div class="pR_content_w">
        <?php if ($cnum_zb==0) {?>
      
        <div class="noneContent">
          <div class="msg yaheiT"><i></i>您还没有预约设计师哦~</div>
          <div class="info">预约工长，您可以免费获得设计师提供的设计和报价方案服务</div>
          <div class="linkA_w"> <a class="linkA" href="/design" target="_blank">进入设计师页面预约</a> </div>
        </div>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
				...
				<tr bgcolor="#eeeeee" align="center" >
				    ...
				    <td width="18%" class="bai">预约设计师</td>
				...
				<?php
				$xxrs=mysql_query("select * from tb_uysj where uid=$arrayuv[id] order by  id desc ",$conn);
				$J=1;
				...
				<tr align="center" onmouseover="style.backgroundColor='#EEEEEE'" onmouseout="style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF">
				    ...
				    <td class="td td3 ">
                   <a href="yz_ysjs_xx.php?id=<?php echo $array["id"]?>"  style="color:#ff0000;"><?php echo $array["bname"]?></a> 
                    </td>
				    <td class="td">
       <?php
	...
	?>				
	<a href="/design/show.php?uid=<?php echo $array5["id"]?>" target="_blank"><span  style="color:#006600;">【<?php echo $array5["relname"]?>】</span></a>
		   ...
		   ?> 
				    ...
				    <td class="td"><input type="button" name="Submit3" value="查看" onclick="window.location.href='yz_ysjs_xx.php?id=<?php echo $array["id"]?>' "  class="btn"/>&nbsp;&nbsp;&nbsp;&nbsp;
				    <input type="button" name="Submit" value="删除" onclick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='yz_ysjs.php?delid=<?php echo $array["id"]?>&del=ok';}else{history.go(0);}"  class="btn"/></td>
				</tr>
				<?php
				$J++;
				 endwhile;
...
$rs=mysql_query("delete from tb_uysj where uid='$arrayuv[id]' and id='$delid'",$conn);
echo "<script>window.location.href='yz_ysjs.php';</script>" ; 
