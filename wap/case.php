...
<title>设计师案例_<?php echo $dnames?></title>
...
<li class="nav-tab "><a   class="<?php if ($bid==0){?>active<?php }?>" href="case.php">全部</a></li>
          
              <?php
	   	$rs2=mysql_query("select  bid,btitle from tb_ustyle  order by  px_id asc ",$conn);
...
if($keys<>""){ 
 ...
 $rrs5=mysql_query("select id   from tb_ushe where id>0 $tiao1 $tiao2 ",$conn);
	 $num=mysql_num_rows($rrs5);
			 ...
			 $p=($page-1)*$pagesize;
			$rsx=mysql_query("select id,img,title,mjs,hits,bid,hprice,hxs,uid from tb_ushe where id>0 $tiao1  $tiao2 order by  id desc limit $p,$pagesize ",$conn);
 if($num>0){ 
 ...
 if ($arrayw['uid']>0) {
 $rs5=mysql_query("select relname,img,jibie  from tb_user where bid=1 and id=$arrayw[uid]",$conn);
...
<div class="case_item">
<a href="case_show.php?id=<?php echo $arrayw['id']?>" title=""><img src="<?php echo $arrayw['img']?>" alt=""></a>
  <div class="tip"><span class="location"><em></em><?php echo $uming?></span><span class="comments"><em></em><?php echo $arrayw['hits']?></span></div>
  <h4><?php echo $tname?></h4>
  <div class="wrap"><span class="fl"><?php echo $arrayw['mjs']?><i>㎡</i></span><span class="fl"><?php echo $arrayw['hxs']?></span><span class="fl"><?php if ($arrayw['bid']>0) {
 $rs5=mysql_query("select btitle  from tb_ustyle where bid=$arrayw[bid]  ",$conn);
 $array5=mysql_fetch_array($rs5);
 echo $array5["btitle"];
 mysql_free_result($rs5);
 }?></span><span class="fr">合同：<em><?php echo $arrayw['hprice']?>元</em></span></div>
</div>
