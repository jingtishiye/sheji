...
<title>设计师加盟_互联网工长装修O2O服务平台</title>
            ...
            <div style="padding:27px 0 0 29px;">
                <form method="post" action="" class="gzhzForm J_ajaxForm"  >
                    ...
                    <div class="lineW">
                        ...
                        <input type="text" class="inp J_delDefVal fullname" id="fullname" placeholder="请填写设计师姓名" maxlength="10">
                        <div class="err"></div>
                    </div>
                    <div class="lineW">
...
<div class="gzhzForemanShow">
    <div class="titleW">
        <div class="t1 yaheiT">他们加入了设计师团队</div>
        <div class="t2">目前已有<span><?php echo mysql_num_rows(mysql_query("select id from  tb_user where bid=1",$conn));?></span>位设计师加盟七工长网</div>
    </div>
...
<div class="mr_frbox">
  ...
  <ul>
   <?php
$rsx=mysql_query("select relname,img,id from tb_user where bid=1 and tjs=1 and sh=1 order by kbid desc,id desc  limit 12",$conn);
while($arrayx=mysql_fetch_array($rsx)):
?>   
   <li>
 
  <div class="pinpic"><a href="/design/show.php?uid=<?php echo $arrayx['id']?>" target="_blank"><img src="../<?php echo $arrayx['img']?>" alt="<?php echo $arrayx['relname']?>" /></a></div>
  <div  class="zname font12"><a href="/design/show.php?uid=<?php echo $arrayx['id']?>" target="_blank"><?php echo $arrayx['relname']?></a></div>
   </li>
  <?php
endwhile;
           ...
           $("#repwd").focus();
			...
			$.alertable.alert('请填写设计师姓名！');
                ...
                $.ajax({
                    ...
                    data: { "type": "addsjs", "uname": uname, "passwd":passwd,"email":email,"utel":tele,"sadd1":sadd1,"sadd2":sadd2,"zname":zname},
